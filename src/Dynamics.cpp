...
Dynamics::StateDerivative Dynamics::StateDerivative::operator+(const Dynamics::StateDerivative& other) const {
...
Dynamics::StateDerivative Dynamics::StateDerivative::operator*(double scalar) const {
    StateDerivative result;
...
Dynamics::Dynamics(const Spacecraft& sc, const World& w) : spacecraft(sc), world(w){
...
Dynamics::StateDerivative Dynamics::computeDerivatives(const Eigen::Vector3d& pos, const Eigen::Vector3d& vel, const Eigen::Vector4d& quat, const Eigen::Vector3d& angVel, const Eigen::Vector3d& bodyForce, const Eigen::Vector3d& bodyTorque) const {
...
void Dynamics::integrateRK4(double dt, const Eigen::Vector3d& bodyForce, const Eigen::Vector3d& bodyTorque) {
    StateDerivative k1 = computeDerivatives(position, velocity, orientation, angularVelocity, bodyForce, bodyTorque);
    ...
    StateDerivative k2 = computeDerivatives(pos2, vel2, quat2, angVel2, bodyForce, bodyTorque);
    ...
    StateDerivative k3 = computeDerivatives(pos3, vel3, quat3, angVel3, bodyForce, bodyTorque);
    ...
    StateDerivative k4 = computeDerivatives(pos4, vel4, quat4, angVel4, bodyForce, bodyTorque);
...
void Dynamics::handleCollisions() { 
...
Eigen::Vector4d Dynamics::getWorldOrientation() const {
    return QuaternionTools::toWorld(orientation);
}

void Dynamics::update(double dt, const Eigen::Vector3d& bodyForce, const Eigen::Vector3d& bodyTorque) {
